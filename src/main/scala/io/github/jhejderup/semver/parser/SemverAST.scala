...
case class PreReleaseTags(pre: Option[List[SemverToken]] = None,
                          build: Option[List[SemverToken]] = None)

case class Semver(major: SemverToken,
                  minor: SemverToken,
                  patch: SemverToken,
                  preRelease: Option[List[SemverToken]] = None,
                  metadata: Option[List[SemverToken]] = None)
    extends SemverAST
    with Ordered[Semver] {
    ...
    case Semver(NUMBER(major), NUMBER(minor), LETTERX | STAR, pre, build) =>
      Intersection(
        List(
          CompareRange(
            GTE,
            Semver(NUMBER(major), NUMBER(minor), NUMBER(0), pre, build)),
          CompareRange(
            LT,
            Semver(NUMBER(major), NUMBER(minor + 1), NUMBER(0), pre, build))))
    case Semver(NUMBER(major), LETTERX | STAR, _, pre, build) if major > 0 =>
      Intersection(
        List(CompareRange(
               GTE,
               Semver(NUMBER(major), NUMBER(0), NUMBER(0), pre, build)),
             CompareRange(
               LT,
               Semver(NUMBER(major + 1), NUMBER(0), NUMBER(0), pre, build))))
    case Semver(LETTERX | STAR, LETTERX | STAR, LETTERX | STAR, pre, build) =>
      Intersection(
        List(
          CompareRange(GTE,
                       Semver(NUMBER(0), NUMBER(0), NUMBER(0), pre, build))))
    case _ => this
  }
  ...
  override def compare(that: Semver): Int = (this, that) match {
    case (Semver(NUMBER(thisMajor),
                 NUMBER(thisMinor),
                 NUMBER(thisPatch),
                 thisPre,
                 _),
          Semver(NUMBER(thatMajor),
                 NUMBER(thatMinor),
                 NUMBER(thatPatch),
                 thatPre,
                 _)) => {
          ...
          if (res == 0) {
            if (!thisPre.isDefined && thatPre.isDefined) {
              ...
              if (thisPreList.size == thatPreList.size) {
                ...
                res = thisPreList
                  .zip(thatPreList.take(thisPreList.size))
                  .toStream
                  .map(_ match {
                    case (NUMBER(t1), NUMBER(t2)) => t1.compare(t2)
                    case (PREID(t1), PREID(t2))   => t1.compare(t2)
...
case class CaretRange(partialRange: Semver) extends SemverAST {
    ...
    case Semver(NUMBER(0), NUMBER(minor), LETTERX | STAR, pre, build) =>
      Intersection(
        List(
          CompareRange(
            GTE,
            Semver(NUMBER(0), NUMBER(minor), NUMBER(0), pre, build)),
          CompareRange(LT, Semver(NUMBER(0), NUMBER(minor + 1), NUMBER(0)))))
  ...
  override def evaluate(that: Semver): Boolean =
    this.transform().evaluate(that)
}
    ...
    case id @ Semver(NUMBER(major), NUMBER(minor), NUMBER(_), _, _) =>
      Intersection(
        List(
          CompareRange(GTE, id),
          CompareRange(LT,
                       Semver(NUMBER(major), NUMBER(minor + 1), NUMBER(0)))))
    case Semver(NUMBER(major), NUMBER(minor), LETTERX | STAR, pre, build) =>
      Intersection(
        List(
          CompareRange(
...
case class CompareRange(op: SemverToken, partialRange: Semver)
    extends SemverAST {
  override def transform(): SemverAST = (op, partialRange) match {
    case (EQU, id @ Semver(_, _, _, _, _)) => id.transform()
    case (LTE,
          Semver(NUMBER(major), NUMBER(minor), LETTERX | STAR, pre, build)) =>
      CompareRange(
        LT,
        Semver(NUMBER(major), NUMBER(minor + 1), NUMBER(0), pre, build))
    ...
    case (GTE, Semver(NUMBER(major), LETTERX | STAR, _, pre, build)) =>
      CompareRange(GTE,
                   Semver(NUMBER(major), NUMBER(0), NUMBER(0), pre, build))
    case (op, Semver(LETTERX | STAR, _, _, _, _)) =>
      CompareRange(op, Semver(NUMBER(0), NUMBER(0), NUMBER(0)))
    case (_, _) => this
  }
...
case class Intersection(rangeSet: List[SemverAST]) extends SemverAST {
  override def transform(): SemverAST =
    Intersection(rangeSet.map(r => r.transform()))
  override def toString: String = rangeSet.mkString(" ")
  override def evaluate(that: Semver): Boolean =
    rangeSet.map(r => r.evaluate(that)).reduceOption(_ && _).getOrElse(false)
